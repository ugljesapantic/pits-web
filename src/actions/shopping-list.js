...
export const shoppingListAddItem = (id, title) => dispatch =>
  api.shoppingList
    .addItem(id, title)
    .then(item => dispatch(itemAdded(item, id)));

export const shoppingListUpdate = (id, body) => dispatch =>
  api.shoppingList
    .update(id, body)
    .then(shoppingList => dispatch(updated(shoppingList)));

export const shoppingListCreate = body => dispatch =>
  api.shoppingList
    .create(body)
    .then(shoppingList => dispatch(created(shoppingList)));

export const shoppingListRemove = id => dispatch =>
  api.shoppingList.remove(id).then(() => dispatch(removed(id)));
